Berikut hasil **deep review** dan versi final yang lebih aman untuk dipakai oleh Agentic AI.

Saya setuju dengan insight Anda: Master Prompt sebelumnya sudah kuat, tetapi untuk eksekusi oleh AI coding agent, kita perlu mengunci beberapa hal lebih keras:

1. **Kriptografi harus Edge-safe** → gunakan **Web Crypto API**, bukan Node native `crypto`.
2. **Tailwind harus CSS-first** → jangan biarkan AI membuat `tailwind.config.ts` gaya Tailwind 3.
3. **Supabase Anonymous Auth tidak boleh membuat user baru saat “Titip Kunci”** → identitas anonim harus di-*upgrade/link*, bukan diganti.
4. **RLS function harus memeriksa otorisasi Inisiator** → `SECURITY DEFINER` tidak boleh menjadi pintu bocor.
5. **Bilik Doa tidak boleh di-cache plaintext oleh PWA** → offline fallback boleh, tetapi jangan simpan refleksi privat dalam bentuk terbaca di IndexedDB.
6. **Service role tidak boleh dipakai di runtime app** → hanya untuk migration/admin job terbatas.
7. **Semua design decision harus token-first** → tidak ada warna, radius, spacing, atau motion hardcoded di komponen.

Untuk tech stack, saya perbarui ke versi terbaru berdasarkan status paket saat ini: Next.js terbaru terdaftar di npm sebagai `16.3.4` [[2]], React sebagai `19.2.8` [[10]], Tailwind CSS sebagai `4.3.3` [[18]], Supabase JS sebagai `2.115.0` [[25]], Supabase CLI sebagai `2.116.0` [[29]], `@supabase/ssr` sebagai `0.12.6` [[33]], TanStack Query sebagai `5.102.8` [[34]], dan Serwist sebagai `9.5.12` [[51]]. Untuk animasi, paket `framer-motion` terbaru terdaftar sebagai `13.2.0`, tetapi karena Motion kini diposisikan sebagai penerus/kelanjutan Framer Motion, instruksi final memakai `motion@latest` dengan import dari `motion/react` bila tersedia [[42]].

---

# 1. REKOMENDASI INFRASTRUKTUR: SUPABASE CLOUD VS SELF-HOSTED

Untuk pilot MVP, rekomendasi saya:

> **Gunakan Supabase Cloud Managed untuk pilot.  
> Rancang skema agar bisa dimigrasikan ke self-hosted jika kelak dibutuhkan.**

## Mengapa Supabase Cloud untuk pilot?

Karena fase pilot perlu memvalidasi:

- apakah Grace Loop bekerja,
- apakah Bilik Doa terasa aman,
- apakah Jendela Gembala cukup berguna,
- apakah PWA cukup stabil di konteks gereja/komunitas.

Pilot tidak boleh habis energinya untuk mengurus:
...
## Kapan self-hosted dipertimbangkan?

Self-hosted masuk akal jika kelak:

- sinode/yayasan besar mensyaratkan kontrol infrastruktur penuh,
- ada kebutuhan data residency khusus,
- ada tim DevOps internal yang matang,
- skala sudah cukup untuk membenarkan biaya operasional self-hosting,
- ada kebutuhan audit/kepemilikan data yang lebih ketat.
...
Post-MVP / Institutional Deployment:
Optional self-hosted Supabase / managed Postgres migration path
```

Aturan penting:

> Jangan pakai fitur proprietary yang membuat data sulit diekspor.  
> Semua schema, migration, RLS, dan function harus hidup di repo.

---

# 2. FINAL MASTER PROMPT

Simpan sebagai:

```text
MASTER_PROMPT_AMANKARSA.md
```
...
Jika memakai Cursor, salin juga ke:

```text
.cursorrules
```

Berikut versi final yang lebih ketat.
...
# 4. TAMBAHAN YANG SAYA SARANKAN UNTUK REPO

Selain `MASTER_PROMPT_AMANKARSA.md` dan `rules.md`, saya sarankan tambahkan file:

```text
docs/CONSTITUTIONAL_CHECKLIST.md
```
...
5. Can a tired person safely stop?
6. Can a person return without shame?
7. Does this measure behavior only, not spiritual worth?
8. Would this feature still be faithful if engagement metrics were removed?

If any answer fails, the PR cannot be merged.
```
...
- spiritual level
- AI reflection analysis
- Bilik Doa admin reader
- hardcoded hex in components
- service role in runtime API
```
...
Master Prompt sebelumnya sudah sangat kuat, tetapi versi final ini menambahkan pagar penting yang lebih realistis bagi Agentic AI:

| Area | Penguatan Final |
|---|---|
| Tailwind | Dikunci ke CSS-first `@theme`, bukan config lama |
| Crypto | Dikunci ke Web Crypto API agar Edge-safe |
| Supabase Auth | Anonymous user harus di-upgrade/link, bukan recreated |
| RLS | `SECURITY DEFINER` wajib authorization check |
| PWA | Dilarang cache Bilik Doa plaintext |
| Service Role | Dilarang di runtime biasa |
| Theme | Light/dark/system sejak awal |
| Design Tokens | Token-first, no hardcoded values |
| AI Boundary | AI tidak boleh membaca/menilai refleksi privat |

Dengan ini, Agentic AI punya pagar yang jauh lebih tajam. Ia tidak hanya tahu *apa yang harus dibangun*, tetapi juga tahu *apa yang tidak boleh disentuh*.
...
Implementation:
Next.js 16 + React 19 + Tailwind CSS 4 + Supabase + RLS + Web Crypto API
...
Dengan setup ini, Amankarsa siap masuk ke **Sprint 0** dengan aman.