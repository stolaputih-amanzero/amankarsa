...
export async function giveRestSpace(journeyId: string) {
  try {
    ...
    console.error('giveRestSpace error:', err);
  }
}
...
export async function createJourney(formData: FormData) {
  try {
    ...
    const defaultPulses: Array<{
      ...
      { day_index: 0, prompt_text: 'Hening sejenak. Sadari kehadiran kasih di sekelilingmu sebelum memulai perjalanan baru ini.', action_type: 'contemplation' },
